...
- 类型：穿越历史（男主角）
- 时期：北魏末年，528-532年
- 主线：现代特种兵穿越到528年，依附尔朱荣势力，参与河阴之变，最终窃取北魏政权
...
5. 设计最终目标：不是自己做皇帝，而是挟天子以令诸侯，建立影子王朝
...
请输出完整的JSON格式世界观设定。"""
    ...
    print("[OK] World setting generated and saved")
    print(f"File: {world_setting_file}")
...
输出格式：结构化JSON，包含title, total_chapters, volumes数组，每个volume包含chapters数组。
...
async def main():
    """主流程"""
    print("\n" + "=" * 60)
    print("《窃魏》Novel Creation System")
    ...
    try:
        ...
        # 保存预览
        preview_file = NOVEL_DIR / "preview.txt"
        preview_content = f"""《窃魏》创作预览
{"=" * 50}
...
"""
        ...
        print("\nGenerated files:")
        print("  - world_setting.json")
        print("  - outline.json")
        print("  - chapters/ch_001.md")
        print("  - preview.txt (summary)")
        print("\nNext steps:")
        print("  1. Check world_setting.json and outline.json")
        print("  2. Read preview.txt for overview")
        print("  3. Continue with Chapter 2")
